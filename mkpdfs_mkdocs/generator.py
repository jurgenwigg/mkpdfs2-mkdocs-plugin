...
from weasyprint import CSS, HTML, urls
from weasyprint.text.fonts import FontConfiguration

from mkpdfs_mkdocs.preprocessor import get_combined as prep_combined
from mkpdfs_mkdocs.preprocessor import get_separate as prep_separate
...
class Generator(object):
    ...
    def set_config(self, local, config):
        ...
        if self.config["design"]:
            ...
            if not os.path.isfile(css_file):
                sys.exit(
                    "The file {} specified for design has not \
                been found.".format(
                        css_file
                    )
                )
    ...
    def write(self):
        if not self.generate:
            self.logger.log(
                msg="Unable to generate the PDF Version (See Mkpdfs doc)",
        ...
        self.logger.log(
            msg="The PDF version of the documentation has been generated.",
            level=logging.INFO,
        )
    ...
    def add_to_order(self, page):
        ...
        if page.is_page:
            ...
            title = self.html.new_tag(
                "h1", id="{}-title".format(uuid), **{"class": "section_title"}
            )
            title.append(page.title)
            article = self.html.new_tag(
                "article", id="{}".format(uuid), **{"class": "chapter"}
            )
    ...
    def add_tocs(self):
        ...
        for n in self.nav:
            if n.is_page and n.meta and "pdf" in n.meta and not n.meta["pdf"]:
                continue
    ...
    def _gen_toc_section(self, section):
        if section.children:  # External Links do not have children
            for p in section.children:
                if p.is_page and p.meta and "pdf" in p.meta and not p.meta["pdf"]:
                    continue
